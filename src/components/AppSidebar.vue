...
<template>
  <nav class="sidebar">
    <div v-for="g in NAV" :key="g.group" class="nav-group">
      <div class="nav-head">
        <span class="ws-ico"><Icon :name="g.icon" :size="13" /></span>{{ g.group }}
      </div>
      <RouterLink
        v-for="it in g.items"
        :key="it.id"
        class="nav-item"
        :class="{ active: it.id === route.meta.navId }"
        :to="it.to"
      >
        <span class="ni-ico"><Icon :name="it.icon" :size="15" /></span>{{ it.label }}
    ...
    <div class="grow"></div>
    <div class="side-foot">
      <Icon name="info" :size="13" /><span>Claude Code 2.1.144 · engine v0.9</span>
    </div>
  </nav>
</template>
