            ...
            <div v-for="it in s.items" :key="it.name" class="card ext" :class="{ disabled: it.disabled }">
              ...
              <div class="em">
                <template v-if="it.override">
                  <span class="tag warn"><Icon name="alert" :size="10" />disabled locally</span>
                  <span class="tag info">enabled by project</span>
                </template>
                <template v-else>
                  <span class="tag ok"><Icon name="check" :size="10" />active</span>
                </template>
        ...
        <div class="card" style="padding: 13px 15px">
          <div class="row" style="gap: 10px">
            ...
            <div class="hint" style="line-height: 1.55">
              Skill-listing budget simulation: at the current <code class="mono-v">skillListingBudgetFraction</code>, all 1 skill
              description fits — none would truncate. Increase skills to preview truncation.
            </div>
          </div>
        </div>
      </div>
