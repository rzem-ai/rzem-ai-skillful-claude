...
const DATA: ConfigRow[] = [
  { key: "model", value: "claude-sonnet-4-6", type: "string", cat: "model", scope: "project", differ: true, conflict: true,
    chain: [
  ...
  { key: "permissions.defaultMode", value: "acceptEdits", type: "enum", cat: "permissions", scope: "project", differ: true, conflict: true, hero: true,
    chain: [
  ...
  { key: "outputStyle", value: "Explanatory", type: "string", cat: "quality", scope: "local", differ: true, conflict: false,
    chain: [{ scope: "local", value: "Explanatory", status: "winner", path: ".claude/settings.local.json", mod: "Jun 6" }] },
  { key: "effortLevel", value: "high", type: "enum", cat: "model", scope: "user", differ: true, conflict: false,
    chain: [{ scope: "user", value: "high", status: "winner", path: "~/.claude/settings.json", mod: "Jun 2" }] },
  { key: "alwaysThinkingEnabled", value: "true", type: "boolean", cat: "model", scope: "user", differ: true, conflict: false,
    chain: [{ scope: "user", value: "true", status: "winner", path: "~/.claude/settings.json", mod: "Jun 2" }] },
  { key: "editorMode", value: "vim", type: "enum", cat: "quality", scope: "user", differ: true, conflict: false,
    chain: [{ scope: "user", value: "vim", status: "winner", path: "~/.claude/settings.json", mod: "Jun 2" }] },
  { key: "env.NODE_ENV", value: "development", type: "string", cat: "environment", scope: "user", differ: true, conflict: false,
    chain: [{ scope: "user", value: "development", status: "winner", path: "~/.claude/settings.json", mod: "Jun 2" }] },
  { key: "env.NPM_TOKEN", value: "npm_4xK9fT2mQ8vLpR3sW7yB1nC5dH6jZ0aE", type: "string", cat: "environment", scope: "user", differ: true, conflict: false, secret: true,
    chain: [{ scope: "user", value: "npm_••••••••••••", status: "winner", path: "~/.claude/settings.json", mod: "Jun 2" }] },
  { key: "forceLoginMethod", value: "claudeai", type: "enum", cat: "security", scope: "managed", differ: true, conflict: false, locked: true, channel: "Enforced via file — /etc/claude-code/managed-settings.json",
    chain: [{ scope: "managed", value: "claudeai", status: "winner", path: "/etc/claude-code/managed-settings.json", mod: "May 12" }] },
  { key: "disableBypassPermissionsMode", value: "disable", type: "enum", cat: "security", scope: "managed", differ: true, conflict: false, locked: true, channel: "Enforced via file — /etc/claude-code/managed-settings.json",
    chain: [{ scope: "managed", value: "disable", status: "winner", path: "/etc/claude-code/managed-settings.json", mod: "May 12" }] },
  { key: "autoConnectIde", value: "—", type: "inert", cat: "ide", scope: null, differ: false, conflict: true, inert: true,
  ...
  { key: "spinnerTipsEnabled", value: "true", type: "boolean", cat: "quality", scope: null, differ: false, conflict: false, isDefault: true,
...
const activeScopes = reactive<Record<ScopeId, boolean>>({
  managed: true, cli: true, local: true, project: true, user: true,
});
...
function vclass(t: string): string {
  return t === "boolean" ? "bool" : t === "string" ? "str" : "";
}
...
function whyText(d: ConfigRow): string {
  ...
  if (d.chain.length > 1) return "Scope order is Managed › CLI › Local › Project › User. The highest scope that defines this key in a valid location wins; lower scopes are shadowed.";
    ...
    <section class="view">
      <div class="view-head">
        ...
        <button class="btn ghost sm" @click="selectKey('permissions.defaultMode')">
          Resolved keys: <b style="margin-left: 4px">{{ DATA.length }}</b>
        </button>
      </div>
      ...
      <div class="view-body">
        <div class="filters">
          ...
          <label class="chk" :class="{ on: onlyDiffer }"><input v-model="onlyDiffer" type="checkbox" />Differs from default</label>
          <label class="chk" :class="{ on: onlyConflict }"><input v-model="onlyConflict" type="checkbox" />Has conflicts</label>
          ...
          <table class="tbl">
            ...
            <tbody>
              <tr
                v-for="d in rows"
                :key="d.key"
                :class="{ sel: d.key === selectedKey }"
                @click="selectKey(d.key)"
              >
                <td>
                ...
                <td>
                  <ProvenanceChip
                    ...
                    :scope="d.scope"
                    :path="d.chain[0].path"
                    :meta="'Last modified ' + d.chain[0].mod"
                  />
                  <span v-else class="dim" style="font-size: 11px">none active</span>
                </td>
                <td>
                  <span v-if="d.locked" class="tag lock"><Icon name="lock" :size="11" />Managed lock</span>
                  <span v-else-if="d.inert" class="shadow-i ignored-i"><Icon name="alert" :size="12" />Ignored · wrong file</span>
                  <span v-else-if="d.isDefault" class="shadow-i"><Icon name="info" :size="12" />Default · source excluded</span>
                  <span v-else-if="d.chain.some((c) => c.status === 'ignored')" class="shadow-i ignored-i"><Icon name="alert" :size="12" />1 shadowed · 1 ignored</span>
                  <span v-else-if="d.chain.some((c) => c.status === 'shadowed')" class="shadow-i"><Icon name="layers" :size="12" />Shadows {{ d.chain.length - 1 }}</span>
      ...
      <template v-if="selected">
        ...
        <div class="insp-b">
          ...
          <div v-if="selected.lint" class="insp-sec">
            <div class="effect warn"><Icon name="alert" :size="14" /><span>{{ selected.lint }}</span></div>
          </div>
          ...
          <div class="insp-sec">
            ...
            <div class="chain">
              <div
                ...
                :key="i"
                class="chain-row"
                :class="c.status === 'winner' ? 'winner' : c.status === 'ignored' ? 'ignored' : 'shadowed'"
              >
                ...
                <div class="chain-main">
                  ...
                  <div v-if="c.note" class="cm-note" :class="{ warn: c.status === 'ignored' }">
                    <Icon v-if="c.status === 'ignored'" name="alert" :size="13" /><span>{{ c.note }}</span>
                  </div>
                  <div v-if="c.action" style="margin-top: 7px">
                    <button class="btn sm" @click="chainAction(c.action)">{{ c.action }} <Icon name="arrow" :size="12" /></button>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div v-if="!selected.locked" class="insp-actions">
          <RouterLink v-if="selected.hero" class="btn primary sm" to="/raw"><Icon name="compare" :size="13" />Open side-by-side</RouterLink>
          <RouterLink class="btn sm" to="/guided/permissions"><Icon name="sliders" :size="13" />Edit in Guided</RouterLink>
          <RouterLink class="btn sm" to="/raw"><Icon name="code" :size="13" />Open in Raw</RouterLink>
        </div>
      </template>
    </aside>
  </main>
...
.sf.off { opacity: 0.32; filter: grayscale(0.5); }
